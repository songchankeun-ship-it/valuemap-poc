...
export function StockDetailActionButtons() {
  ...
  return (
    <nav aria-label={t.navAriaLabel} className="w-full">
      <ul className="grid grid-cols-1 min-[380px]:grid-cols-2 xl:grid-cols-4 gap-2 sm:gap-3 list-none">
        {ACTIONS.map(({ label, Icon, href }) => (
          <li key={href}>
            <a
              href={href}
              className="min-h-[44px] w-full inline-flex items-center justify-center gap-2 rounded-lg border border-zinc-200 dark:border-zinc-700 bg-white dark:bg-zinc-900 px-3 py-2 text-center text-xs md:text-[13px] font-semibold text-zinc-900 dark:text-zinc-100 shadow-sm hover:border-blue-400 dark:hover:border-blue-700 hover:bg-blue-50 dark:hover:bg-blue-950/30 transition"
            >
