...
const EXPLANATIONS: Record<ScoreKind, ScoreExplanation> = {
  ...
  vol: {
    title: "위험조정 · 과거 수익-변동성 효율",
    oneLine: "과거 수익을 변동성으로 나눈 효율 — 안전(저위험) 점수가 아닙니다.",
    detail: "관측 기간에 '출렁임 대비 얼마나 효율적으로 벌었나'를 봅니다(Sharpe 유사). 같은 수익률이어도 출렁임이 적으면 점수가 높음. 점수가 높다고 변동성이 작거나 안전하다는 뜻은 아닙니다.",
    howToRead: "70 이상은 과거 변동성 대비 수익 효율이 좋았던 구간 — 앞으로의 안전을 보장하지 않습니다. 30 이하는 변동성 대비 수익이 부진.",
    caveat: "이 점수는 '안전' 지표가 아니에요 — 실제 위험은 아래 '위험 상세'의 절대 변동성·최대낙폭으로 함께 확인하세요. 표본 기간이 짧거나 거래가 적으면 변동성이 낮아 보이는 착시도 생길 수 있어요.",
  },
};
