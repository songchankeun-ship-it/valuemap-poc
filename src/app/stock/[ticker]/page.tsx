...
export default async function StockDetailPage({ params }: PageProps) {
  ...
  return (
    <div className="space-y-3 md:space-y-4">
      ...
      <nav className="text-[11px] md:text-xs text-gray-500 flex items-center gap-1">
      ...
      <header className="space-y-2">
        <div className="flex items-center gap-2 flex-wrap">
          <h1 className="text-lg md:text-xl font-semibold">{s.name}</h1>
          <span className="text-[11px] px-2 py-0.5 bg-gray-100 text-gray-600 rounded-md tabular-nums">{s.ticker}</span>
        </div>
        <div className="flex items-baseline gap-2 flex-wrap">
          <span className="text-xl md:text-2xl font-semibold tabular-nums">{s.currentPrice.toLocaleString()}<span className="text-sm font-normal text-zinc-500 ml-0.5">원</span></span>
          <span className={"text-sm font-medium tabular-nums " + (s.changePct >= 0 ? "text-red-600" : "text-blue-600")}>
      ...
      <section className="grid grid-cols-2 md:grid-cols-4 gap-2">
        {[
          ...
          <div key={m.l} className="bg-gray-50 rounded-md p-2.5 md:p-3">
            <div className="text-[10px] md:text-[11px] text-gray-500 mb-0.5">{m.l}</div>
            <div className="text-base md:text-lg font-semibold tabular-nums">{m.v}</div>
          </div>
        ))}
      </section>
      ...
      <section className="bg-white border border-gray-200 rounded-lg p-3 md:p-4">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-2 md:gap-3">
          {[
            ...
            <Link key={x.l} href={"/guide/metrics#" + x.anchor} className="block hover:bg-zinc-50 -mx-1 px-1 py-1 rounded transition">
              <div className="text-[11px] md:text-xs text-gray-600 mb-1">{x.l}</div>
              <div className="text-base md:text-lg font-semibold tabular-nums">{x.sc.toFixed(0)}</div>
        ...
        <div className="mt-3 p-2.5 md:p-3 bg-blue-50 rounded-md">
