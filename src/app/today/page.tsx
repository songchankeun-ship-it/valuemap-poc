...
function riskNote(s: { value: number; vol: number }, r3m: number | null): string {
  if (r3m !== null && r3m >= 80) {
    // 급등 정도·변동성에 따라 문구를 나눠 같은 주의 문장이 반복되지 않게 한다(홈과 동일·모두 확인·검토 톤).
    ...
    <div className="space-y-5 md:space-y-7">
      ...
      <section className="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-lg p-3 md:p-5">
        ...
        <ul className="space-y-1.5 text-[12px] text-zinc-600 dark:text-zinc-300">
          ...
          <li className="flex gap-2"><span className="text-blue-500 dark:text-blue-400 shrink-0 tabular-nums">2.</span><span>최근 단기 급등 여부와 <strong className="text-zinc-800 dark:text-zinc-100">상승 원인·지속 가능성</strong>을 살펴봤나요?</span></li>
