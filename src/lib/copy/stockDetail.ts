...
export const stockDetailCopy = {
  ko: {
    ...
    metricsSection: {
      ...
      scoreVsRankNote: (n: number) => `점수는 0~100 비교 척도(추세·밸류는 유니버스 내 백분위) · 순위는 현재 ${n}종목 안 상대 위치예요.`,
  ...
  en: {
    ...
    metricsSection: {
      ...
      scoreVsRankNote: (n: number) => `The score is a comparative 0–100 scale (trend and valuation are universe percentiles); the rank is the relative position within the current ${n} stocks.`,
...
export const priorityScoreCardCopy = {
  ko: {
    ...
    scoreVsRankNote: (n: number) => `점수는 0~100 비교 척도(추세·밸류는 유니버스 내 백분위) · 순위는 현재 ${n}종목 안 상대 위치예요. 점수가 중립이어도 순위는 상위일 수 있어요.`,
    badgeAriaLabel: "데이터 상태 배지",
    trustLabel: "입력 데이터 완성도",
    completenessNote: "완성도는 입력값이 채워진 정도일 뿐, 원본 데이터의 정확성이나 향후 수익을 보장하지 않습니다.",
    requiredDataPrefix: "필수 데이터",
    suspectPill: "이상값 점검 중 · 임시 점수",
    passPill: "이상값 점검 통과",
    confidenceGloss:
      "‘필수 데이터’는 점수 계산에 필요한 값이 얼마나 채워졌는지, ‘이상값 점검’은 가격 데이터에 튀는 값이 없는지 자동 확인한 결과예요. 완성도가 높아도 원본 데이터의 정확성이나 향후 수익을 보장하지는 않습니다. 점검 중인 종목은 임시 점수라 공식 후보·순위에서 빠집니다.",
  },
  en: {
    ...
    scoreVsRankNote: (n: number) => `The score is a comparative 0–100 scale (trend and valuation are universe percentiles); the rank is the relative position within the current ${n} stocks. A neutral score can still rank near the top.`,
    badgeAriaLabel: "Data status badges",
    trustLabel: "Input-data completeness",
    completenessNote: "Completeness is only how much input is filled in — it does not guarantee source-data accuracy or future returns.",
    requiredDataPrefix: "Required data",
    suspectPill: "Outlier check in progress · provisional score",
    passPill: "Passed outlier check",
    confidenceGloss:
      "‘Required data’ is how much of the values needed to compute the score are filled in; ‘outlier check’ is an automated check for spikes in the price data. Even at high completeness, this does not guarantee source-data accuracy or future returns. Stocks still under check carry a provisional score and are left out of official candidates and rankings.",
  },
} as const satisfies Record<Locale, unknown>;
...
export const scoreBasisCopy = {
  ko: {
    ...
    scoreVsRankRest:
      "— 점수는 0~100 비교 척도이며(추세·밸류는 현재 유니버스 내 백분위라 종목 구성이 바뀌면 값도 달라집니다), 순위는 다른 종목과 견준 상대 위치입니다.",
  ...
  en: {
    ...
    scoreVsRankRest:
      "— the score is a comparative 0–100 scale (trend and valuation are percentiles within the current universe, so their values shift if the pool changes), while the rank is the relative position against other stocks.",
...
export const stockChecklistCopy = {
  ko: {
    ...
    reset: "전체 지우기",
    nextCta: (label: string) => `다음 확인: ${label}`,
    resumeHint: (label: string) => `오늘은 여기까지 해도 돼요. 다음 방문에는 "${label}"부터 이어서 확인할 수 있어요.`,
    allDoneHint: "이번 종목 루틴을 끝냈어요. 다음 방문에는 관심 화면에서 다시 꺼내볼 수 있어요.",
  ...
  en: {
    ...
    reset: "Clear all",
    nextCta: (label: string) => `Next: ${label}`,
    resumeHint: (label: string) => `You can stop here today. Next time, continue from "${label}".`,
    allDoneHint: "This stock routine is complete. Next time, reopen it from your routine view.",
