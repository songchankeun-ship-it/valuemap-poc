...
// '거래활성도 급증' 전용 실데이터 소스가 아직 없어, 최근 거래량 비율(flowStats.ratio,
// 최근 5일 평균 거래량 / 20일 평균 거래량)로 대체 추정한다. ratio가 결측이면 거래활성도 점수(flow)로 폴백.
// 입력은 주식 체결 수량(거래량)이며 거래대금(원)이 아니다 — 계약 정본은 realStocks.ts flowStats 주석.
// 실제 거래량 급증 데이터가 생기면 이 헬퍼만 교체하면 된다(파생 추정값임을 라벨로 명시).
import type { RealStock } from "./realStocks";
import { isSuspect } from "./dataQuality";

// 최근 거래량이 평소(20일 평균)의 1.5배 이상이면 '거래 관심 증가'로 본다.
