...
// ✅ 이 파일은 현재 서비스의 실제 운영 산식(Metrics 2.4)과 동일합니다. 옛 POC 구현이 아닙니다.
//
// 산식 버전: Metrics 2.4 (전역 dataStatus.metricsVersion 과 일치)
// 적용일: 2026-06-14
//
// ⚠ 모든 점수(0~100)는 '지금 더 자세히 볼 종목'을 고르는 탐색 우선순위이며,
//    매수·매도 신호가 아닙니다. 점수가 높다고 매수 추천이 아닙니다.
//
// 운영: 영업일 장마감 후 배치(scripts/compute_metrics.py)가 Naver Finance 전달/FDR 수집 일별 종가로 전 종목 점수를
...
//  위험조정(vol)    : 최근 1년 (연환산수익 − 무위험 3.5%) / 연환산변동성 → 50 + 샤프×25
//  종합(composite)  : 네 지표 단순 평균 (가중치 없음)
// ───────────────────────────────────────────────────────────
//
// ── 버전 경계(정직한 라벨) · @metricsVersionBoundary 2.4-public ────────────────
// 이 모듈은 현재 공개 사이트가 쓰는 Metrics 2.4 호환 계산 경로다(공개 정본 = 2.4, 유지).
// 2.5.1 shadow 소비자는 이 파일을 import 하지 않는다: 2.5.1 최종 점수는 Python
// 엔진이 확정 저장하고 scripts/metrics251/ 의 reader 계약(재계산 없음)으로만 읽는다.
// 새 2.5.1 코드에서 이 2.4 산식을 재사용하면 "두 개의 진실"이 생기므로 정적 가드
// (scripts/test_metrics251_contracts.ts)가 scripts/metrics251/* 의 import 를 차단한다.
// 근거: docs/ornscore-metrics-v2.5.1-amendment-2026-07-15.md §3.1 · Slice J.
// ─────────────────────────────────────────────────────────────────────────────
