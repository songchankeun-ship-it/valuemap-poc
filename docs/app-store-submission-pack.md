...
- 수집 데이터 후보: 이메일 주소, 이름/닉네임, 프로필 사진(소셜 제공자가 전달하는 경우), 소셜 로그인 식별자, 관심 종목, 비교 목록, 알림 설정, 접속 정보, 이용자 제출 정보(데이터 오류 제보 내용·회신용 이메일, 출시 대기 신청 이메일).
- 로그인 없이 수집되는 항목 주의: 데이터 오류 제보 폼(`/status`)과 출시 대기 신청 폼은 비로그인 상태에서도 제출 가능하며, 제보 내용·선택 이메일·대기 신청 이메일이 Supabase(`data_reports`·`waitlist`)에 저장된다. Data safety에서 "User content"·이메일 수집 항목을 이 흐름까지 포함해 표기한다.
- 사용 목적: 계정 관리, 로그인, 관심 종목 동기화, 알림 발송, 서비스 품질 개선, 오류 제보 회신·문의 처리, 출시 안내.
...
- Contact Info: 이메일 주소(로그인 계정, 데이터 오류 제보 회신용 선택 입력, 출시 대기 신청 포함), 이름/닉네임. 로그인·계정 식별·알림 발송·문의 회신 목적.
- User Content 또는 Other User Content 후보: 관심 종목, 비교 목록, 알림 설정, 데이터 오류 제보 내용(카테고리·종목 코드·메시지). 사용자 계정 기능 제공 및 문의 처리 목적.
