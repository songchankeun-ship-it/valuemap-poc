...
## 0. 2026-07-01 현재 권장 경로

- **기본 추천**: 지금은 PWA 실기기 QA를 먼저 끝낸 뒤, 스토어 노출이 필요하면 **Android TWA를 1순위**로 진행한다. 이유는 현재 Next/PWA 코드와 운영 배포를 거의 그대로 재사용하면서 Play 스토어 등재를 실험할 수 있기 때문이다.
- **iOS 정식 래퍼는 후순위**: App Store는 단순 웹 래퍼 반려 리스크가 있어, PWA/Android TWA에서 로그인 복귀·관심종목·알림 설정·법적 고지 UX를 먼저 검증한 뒤 검토한다.
...
- **패키지명 후보**: Android TWA를 진행한다면 우선 후보는 `com.ornscore.app`이다. 단, Play Console 앱 생성 전 운영자가 최종 확정해야 하며, 한 번 출시한 패키지명은 사실상 바꾸기 어렵다.
