...
## 0. 2026-07-01 제품 결정 — Android TWA 우선

- **결정**: 오른스코어 앱 1차 패키징은 **Android TWA를 우선**한다. PWA 실기기 QA를 먼저 끝낸 뒤, Play Console·서명 지문·assetlinks를 준비해 Android Play Store 등재를 진행한다.
- **이유**: 현재 Next/PWA 코드와 운영 배포를 거의 그대로 재사용하면서 Play 스토어 등재를 실험할 수 있어, 새 네이티브 구조를 들이는 것보다 개발·심사 리스크가 낮다.
- **iOS 정식 래퍼는 보류**: App Store는 단순 웹 래퍼 반려 리스크가 있어, Android TWA와 실사용 피드백 이후 검토한다. 그 전까지 iOS는 홈 화면 추가 PWA로 운영한다.
- **현재 앱 상태와 제출 초안 정합**: `docs/app-store-submission-pack.md`는 무료 베타·유료 결제 없음·AI 공개 비노출·Kakao/Google/Naver 로그인 활성 상태에 맞춰 갱신했다. 이후 인증 제공자·개인정보 항목·AI 공개 여부가 바뀌면 이 문서와 `app:check`를 함께 갱신한다.
- **패키지명 결정안**: Android TWA 패키지명은 `com.ornscore.app`을 기본값으로 한다. 단, Play Console 앱 생성 전 운영자가 최종 확인해야 하며, 한 번 출시한 패키지명은 사실상 바꾸기 어렵다.
...
### 1-2. Android TWA (결정된 1차 스토어 경로) — 가장 적은 추가 작업으로 Play 등재
- **제품 결정**: 첫 스토어 경로는 Android TWA로 잠금. 남은 인간 결정은 Play Console 개발자 등록·앱 생성·서명 키 관리 방식이다.
