...
| O. QA 체크리스트(§47~53) | ② | #38 데스크톱/390px 시각 QA 스윕에서 12경로 점검(코드 0수정·치명 오류 0). **실 브라우저 모바일 게이트는 운영자(⑤)**. **Task 154(피드백 인테이크 루프 드라이런)**: `docs/ornscore-expert-feedback-intake-template.md`(Task 129) 루프를 SAMPLE 리포트로 1회 실행 → `docs/ornscore-expert-feedback-dry-run-2026-07-03.md`(거부/P2 방출/운영자 버킷/중복 드롭 4경로). 템플릿 §7에 dedup("이미 추적/완료?")·"운영자 전용?" 칸 additive 추가로 중복 task 방지 강제. 문서 전용(소스 무변경) · **Task 166(오너 리뷰 패키지)**: 배치 144~165를 오너 리뷰용으로 종합 — `docs/ornscore-owner-review-2026-07-03.md`(§1 배치 요약 표·§2 8도메인 체크리스트[UX·데이터 신뢰·모바일·로그인·관심·알림·성능·재무 문구, 각 상태+검증 명령]·§3 다음 로컬 제안 ③ 5건·§4 운영자·법무 게이트 링크). 문서 전용(소스 무변경) · **Task 177(로컬 릴리스 근거 팩)**: 외부 피드백 노출 go/no-go 판단용으로 종합 — `docs/ornscore-local-release-evidence-2026-07-03.md`((a) 판단 프레임 A치명결함/B문구불변식/C실기기·법무·(b) 오너 리뷰 이후 자동화 167~176 요약 표(144~165는 owner-review 링크)·(c) 스모크7+수동11 라우트 링크·(d) 남은 리스크 ③/④/⑤ 7건 링크·(e) 오너 최종 체크리스트 B절 링크·(f) 9도메인 로컬 릴리스 체크리스트·(g) 검증 명령 게이트/권고). 문서 전용(소스 무변경) · **Task 189(한국어 우선 카피 정리)**: 무료 한국어 베타 공개 표면 감사(혼재 언어·유휴 언어 컨트롤·용어 드리프트·첫 방문자 문구) → `docs/ornscore-korean-first-copy-cleanup-2026-07-03.md`. 결과: KO/EN 토글 이미 숨김(EN 보존)·혼재 누수 0·첫 방문 문구 어색 0. **Fix-now 7건** = 지표 라벨 캐논 정렬(추세·거래활성도·밸류·위험조정): `theme/[slug]` 레이더·`page.tsx`/`stock/[ticker]` meta·OG·`mockStockPool` 내부 정렬 라벨. 글로서리 브리지·SEO 키워드·미사용 `ScoreTooltip`은 유지. tsc0·verify138/0/0·build0·smoke7/7·홈SSR 한국어 전용 확인. 점수식/데이터/`direction` 무변경 · **Task 190(검색 미리보기·페이지 메타데이터 하드닝)**: 공개 페이지 title/description/OG·Twitter/canonical/robots 일관화 → `theme/[slug]` 신규 `generateMetadata`(중립 데이터 프레이밍, 추천 아님)·`stock/[ticker]` canonical 추가·정적 공유 8페이지(about·today·disclosures·backtest·pricing·guide/metrics·compare·universe) OG/Twitter/canonical·`stocks` 두 브랜치 OG(canonical은 기본 브랜치만)·private 4페이지(watchlist·history·settings/notifications·**login은 서버 래퍼+`LoginContent` 분할**) robots noindex. `today` 얇은 문구 개선. **회귀 수정**: 중첩 라우트 `openGraph` 정의가 루트 `opengraph-image.tsx` 상속을 끊어 공유 이미지 소실 → 편집 페이지 전부 `images:["/opengraph-image"]`로 기존 공용 카드 유지(신규 이미지 생성 0). tsc0·verify138/0/0·build0(176 SSG)·smoke7/7·SSR head 스팟체크 정합. **페이지별 맞춤 공유 카드 이미지는 여전히 ⑤ 오너/디자인 게이트**(Task 145 §3 동일). 점수식/데이터/`direction` 무변경 · **Task 195(유니버스 무결성 가드 + /universe 상태 표기)**: 읽기 전용 진단(138종목 → 중복 티커 0·중복 종목명 0·필수 필드 결측 0·`sectorOf` 미분류(기타) 0·`isSuspect` 15종목)으로 실제 결함만 대상화. (a) `scripts/verify_metrics.py`에 additive 무결성 가드 — 중복 티커/종목명 검출 + 필수 필드(name·market·비어있지 않은 themes·currentPrice>0·marketCap>0) 결측 검출을 기존 `errors` 목록에 합류(단일 `sys.exit`·`오류 M건` 요약·FORBIDDEN/산식 버전 게이트 무변경). (b) `/universe`(`src/app/universe/page.tsx`) 표시 하드닝(CSS/문구만) — `isSuspect` 종목에 `검증보류` pill + 탐색 우선도 셀 회색 처리(오늘·상위 목록 제외 방식과 정합)·업종 `기타`→`기타(미분류)`·표 하단 KRX 내부 분류 캡션(`CompareClient` 문구 재사용). (c) `rows.length===0` 빈 상태 "데이터 준비 중" 패널(기존 muted 카드 스타일 재사용)로 데이터/빌드 글리치 시 graceful degrade, 신규 마크업은 `overflow-x-auto` 래퍼 내부·additive/`md:` 클래스로 390px 모바일 무영향. tsc0·verify138/0/0(신규 중복/결측 가드 포함)·build·smoke7/7. 점수식/데이터/`direction`·`sectorOf`/`isSuspect` 로직 무변경(문구·표시만) · **Task 191(출시 관찰성 체크리스트)**: 무료 베타 노출 후 오너가 로컬에서 돌려볼 관찰성 참고표 `docs/ornscore-launch-observability-checklist.md` 신규 — 4절(핵심 라우트 이름[공개12+내부`/admin/status`+API/cron7]·모니터링할 사용자 액션[Vercel Analytics/Speed Insights 기존 배선·신규 SDK 0]·헬스 신호[`dataStatus.selfCheck`·`isDataStale`/`asOfBusinessDate`·cron·`/status`vs`/admin/status`·`data_reports`]·수동 리뷰[build→next start 4455→smoke:check 게이트/exit1·perf:check 권고·390px육안·포트가드레일])+완료기준. 스모크·QA 문서는 링크만(중복 0). 하니스: `/admin/status` 푸터 노트에 신규 체크리스트 링크 1줄 additive(레이아웃/타입 무변경). 문서 전용+트리비얼 JSX 1줄 · 점수식/데이터/`direction` 무변경 · **Task 198(모바일 뷰포트 후속 감사)**: 6개 라우트(home·today·stocks·stock상세·compare·login)+공용 크롬을 360/390px 다섯 결함 클래스(헤더겹침·넘침·밀집컨트롤·44px탭타깃·CLS)로 소스 대조 감사 → `docs/ornscore-mobile-viewport-followup-2026-07-04.md`. 결과: 넓은 표 전부 `overflow-x-auto`(StockResultsTable·Compare* sticky 라벨 열·MonthlyHeatmap), 탭 타깃 `min-h-[44px]`, login CLS 스켈레톤, 하단네비 `pb-[calc(4rem+safe-area)]` 이미 정상 → **소스 무변경(불필요 churn 회피)**. 유일한 실질 발견 = `StockTabs.tsx:53` 탭 바 `sticky top-0 z-10`가 앱 헤더 `sticky top-0 z-40`와 동일 `top:0`·하위 z라 스크롤 시 헤더 뒤로 가려짐(저심각·sticky no-op). 올바른 오프셋은 헤더높이+`env(safe-area-inset-top)`(브레이크포인트·노치 의존)라 실기기에서만 검증 가능 → **owner⑤ 게이트로 기록**(강제 매직넘버 회피). tsc0·verify138/0/0·build0·smoke12/12·6라우트 SSR 앵커 확인. **실 브라우저 모바일 픽셀 육안은 여전히 운영자 ⑤**. 점수식/데이터/`direction` 무변경 · **Task 193(로컬 스모크 커버리지 확장)**: `scripts/smoke-check.mjs` `--all` 세트를 12→23 라우트로 확장(기본 게이트는 유한 7 무변경). (a) 라우트별 optional `expectStatus`(기본 200) additive 추가 — 상태 단언을 `status !== (route.expectStatus ?? 200)`으로, 기존 라우트 전부 암묵 200 유지(하위호환). (b) 런칭 핵심 공개 페이지 7개(`/about`서비스 소개·`/guide/metrics`지표 가이드·`/guide/metrics/changelog`산식 변경 이력·`/universe`분석 대상·`/terms`이용약관·`/privacy`개인정보처리방침·`/theme/battery`2차전지[mockData 실존 슬러그])를 SSR 앵커와 함께 추가. (c) 부정/폴백 그룹 — `/__no_such_route__`는 하드 404(`expectStatus:404`·앵커 `찾을 수 없습니다`), `/stock/000000`은 SSG `notFound()`가 200+not-found 본문으로 내려주는 **소프트 404**(000000/999999/ZZZZZZ에서 안정 재현)라 상태 대신 본문+무크래시 검증. (d) 비로그인 플로우 — `/history`·`/settings/notifications`가 리다이렉트/에러 없이 200으로 `로그인` CTA 렌더(보호 라우트 graceful degrade). 모든 앵커는 SSR 소스 대조 확인. `docs/ornscore-route-smoke-checklist.md` `--all` 표+부정/폴백+비로그인 서브섹션 갱신(기본 7 유한 재확인). 게이트: tsc0·verify138/0/0(데이터 무변경)·build0·`--all` 23/23 OK·기본 7/7 무변경. 앱 소스/점수식/데이터/`direction` 무변경(스크립트+문서 전용) · **Task 194(오너 리뷰 패키지 178~198)**: 배치 178~198을 오너 리뷰용으로 종합 — `docs/ornscore-owner-review-2026-07-04.md`(§1 5테마 배치 요약 표[개인화·탐색공시·신뢰문구·로그인모바일·QA게이트, task별 성격 docs/source·한 줄·근거 커밋/문서]·§2 9도메인 수동 검증 체크리스트[192 에러바운더리·193 --all 23라우트·195 유니버스 무결성·196 상대시각·197 교육 카피·198 모바일뷰포트 갱신]·§3 3축 go/no-go[A·B 로컬 GO·C 오너 게이트, 비파괴 외부/베타 GO·스토어/결제 오너 뒤]·§4 ④/⑤ 게이트 링크). 144~165·167~176은 앞선 두 패키지(owner-review 07-03·local-release 07-03)로 링크만. tsc0·verify138/0/0·Metrics 2.4·신규 .md U+FFFD 0. 문서 전용(소스 무변경) · **Task 219(실기기 390px 릴리스 QA 런북)**: 무료 베타 공개 전 오너가 실 모바일(390×844·다크/라이트)에서 혼자 통과시키는 실행형 체크리스트 → `docs/ornscore-real-device-390px-qa-2026-07-06.md`(§0 불변식 배너·§1 자동 프리-패스[build→next start 고포트→smoke:check --all→verify:routes]·§2 실기기 셋업·§3 7라우트(`/`·`/stocks`·`/stock/005930`·`/status`·`/pricing`·`/login?next=/watchlist`·`/watchlist`) 기대화면/모바일리스크/합격칸·§3 고위험 `StockTabs.tsx:53` sticky occlusion A/B 오너 결정). SSR 자동검증은 기존 smoke/verify가 커버 → 신규 스크립트/의존성 0(과설계 회피), 남은 것은 실 픽셀 육안 ⑤ 게이트. 문서 전용(소스/점수식/데이터/`direction` 무변경) · **Task 220(사이트 새로고침 모니터링·롤백 노트)**: 릴리스 라인을 되돌릴 수 있고 관찰 가능하게 만드는 첫 노트 → `docs/ornscore-site-refresh-monitoring-rollback-2026-07-06.md`(§0 불변식 배너+상류 노트[`origin/main...HEAD = 0 112`·codex 통합 라인 포함하나 HEAD 아님]·§1 새로고침 직후 즉시 확인[자동 `verify:routes` 6라우트→`smoke:check --all` 23라우트→수동 하드리로드 패스(상태/기준일·`/login` 진입·`/pricing` 무료 베타 카피·KO/EN 토글 부재)+오너 제공 URL 슬롯]·§2 롤백 트리거→액션 표[치명마커/5xx/stale기준일/유료프레이밍재부상/KO-EN누출/로그인깨짐⇒롤백, 경미⇒모니터+기록]+안전 롤백 노트[Vercel promote 또는 `git revert -m 1 <merge-sha>`, `public/data/*`·점수식·`direction`·`metricsVersion` 무접촉]·§3 PROGRESS 캡처 목록·§4 남은 오너 승인[배포/새로고침·OAuth 왕복·URL 제공·promote/revert]·§5 신규 스크립트 미추가 근거[verify:routes가 이미 배포 URL 자동화]). 문서 전용(소스/점수식/데이터/`direction`/`metricsVersion` 무변경) |
...
| 8.2 관심 종목 그룹/메모/CSV | ③/④ | `watchlist.ts` 추가/삭제·점수 델타 존재. 그룹·메모·CSV는 신규 기능. **Task 160**: 관심 종목 리텐션 루프 폴리시(알림 이전에도 담기·재방문이 가치 있게) — `AddToWatchlistButton.tsx` 추가/제거 토스트 분기(추가 2.5s·제거 5s+`실행 취소` 되돌리기, aria-live 상시 렌더, 모바일 `MobileBottomNav` 위 `bottom-[calc(3.5rem+safe-area)]`·`lg:bottom-6`, 44px 탭타깃·언마운트 타이머 정리), `WatchlistClient.tsx` 리스트 인라인 제거 `실행 취소`(5초·compare와 동일 패턴)·제거 aria-label 종목명 포함·삭제 버튼 44px·"내 현황/빈 상태" 카피, **정직 프레이밍** 강화("담기=별도 알림 없이 로컬 기록, 메일 알림은 이메일만 임시·베타, 카카오톡·푸시 준비 중" — 라이브 알림 배선 암시 0·`매수/매도/추천/수익 보장` 0), `watchlist/page.tsx` 헤더 로컬 추적 1줄 추가. 점수식/데이터/인증/알림 배선 무변경·제3자 호출 0. **Task 196(최근 본 종목 재방문 큐 + 상대시각 유틸 통일)**: 두·세 번째 방문에서 "언제 봤는지"를 보여줘 리텐션 경로 강화. (a) `recentViews.ts` `RecentView.viewedAt` 타입 `string`→`number` 교정(`RecentViewTracker`가 `Date.now()` 숫자로 기록하는 것과 일치)·`getRecentViews` 읽기 시 숫자/레거시 ISO 문자열 모두 `number`(ms)로 정규화 + 형식 깨짐·비유한값 항목 드롭(레거시 키 폴백·10개 캡 유지). (b) `format.ts`에 공통 `fmtRelativeTime(input, {locale, absolute})` 추가(Invalid Date 방어, ko 방금 전/N분·시간·일 전, en just now/Nm·Nh·Nd ago, 7일+ `toLocaleDateString` 폴백 `md`=월/일·`ymd`=연/월/일) → `WatchlistClient`(`md`, `useLanguage` locale)·`HistoryClient`(`ymd`) 로컬 `formatTime` 삭제하고 채택(ko 출력 바이트 동일). (c) `MyStocksSection` 최근 본 행에만 `viewedAt` 전달해 업종 대신 `최근 본 · N분 전` 표시(관심 종목 행은 업종 유지)·중립 톤·`mounted` 하이드레이션 가드·44px·저장소 예외 try/catch 유지. tsc0·verify138/0/0·build(138 SSG 유지)·smoke12/12. 점수식/데이터/`direction` 무변경(표시·타입만). 그룹·메모·CSV는 여전히 신규 기능(④) · **Task 221(관심 그룹·메모·CSV 문서 우선 설계서)**: 다음 제품 베팅 #1을 구현 없이 설계서로 확정 → [`ornscore-watchlist-groups-memo-csv-spec.md`](./ornscore-watchlist-groups-memo-csv-spec.md)(§0 무료 KO 베타 불변식+로컬 우선/프라이버시 배너·§1 현 상태 감사[CSV 유틸 부재 확인]·§2 데이터 모델[`savedSearches.ts` 이중 저장·`crypto.randomUUID`·캡·`*-changed` 재사용 — 그룹 `{id,name}`+`groupId`·메모 500자]·§3 CSV 컬럼 계약 ticker,name,group,note,addedAt,compositeScore[표시용 스냅샷·비자문]·§4 MVP(반출 전용+인라인 그룹+메모)/후속(가져오기·rename·동기화) 분리·§5 프라이버시 리스크[클라이언트 Blob 다운로드만·UTF-8 BOM·CSV 수식 인젝션 방역·PII 유도 0·금칙어 리뷰]·§6 첫 슬라이스=**CSV 반출 전용(S)**·§7 검증 게이트·§8 교차참조). 문서 전용·소스/점수식/`direction` 무변경·오너 게이트는 Supabase 마이그레이션·구현 착수뿐 |
...
| 20.4 폴백 상태(결측·에러·오프라인·404) | ✅ (task 176) | **Task 176(회복 탄력적 폴백)**: `not-found.tsx` 다크모드 판독성 버그 수정(전 색상 `dark:` 변형·버튼 44px)+i18n, `offline/page.tsx` i18n(둘 다 `metadata` 서버 래퍼 + `"use client"` 자식 `NotFoundContent`/`OfflineContent`), `i18n.ts` `notFoundCopy`/`offlineCopy` 신규, `StockDisclosures` 에러 브랜치를 `DisclosureExplorer`와 시각 정렬(AlertTriangle+rose 카드+`t.loadError`/`t.loadRetry`). 나머지 폴백 표면(DisclosureExplorer 빈/에러·Stocks 검색0건·Watchlist·Compare·AiAnalysis·ScoreHistory<10·Sector 저표본)은 이미 의도적 프레이밍 → keep-as-is. CSS/카피/i18n 한정·데이터/점수/`direction` 무변경. 실 브라우저 다크모드·EN 토글 육안은 운영자(⑤). **Task 192(에러 바운더리·오프라인 재시도)**: 예외 시 빈 화면 방지 — `app/error.tsx`(route-segment 바운더리, `LanguageProvider`/`ThemeProvider` 컨텍스트 사용)+`"use client"` `ErrorContent`(AlertTriangle 카드·`다시 시도`→`reset()`·`홈으로`·44px·`dark:`), `app/global-error.tsx`(루트 layout 대체용 최후 바운더리, 자체 `<html lang="ko"><body>`·컨텍스트 무의존·한국어 고정 카피), `OfflineContent`에 `다시 시도`(→`window.location.reload()`, SSR 가드) 버튼 추가, `i18n.ts` `errorCopy`(ko/en) 신규+`offlineCopy.retryButton`. 금칙어 0(시스템 상태 안내만). 게이트: tsc 0·verify_metrics 138/0/0·build·smoke 12/12·`/offline` SSR 한국어+재시도 버튼 확인. 에러 바운더리는 예외 발생 시에만 트립되므로 런타임 육안 검증은 빌드/타입 성공+오프라인 스팟체크로 한정(⑤) |
| 라이트 모드 전역 토큰 도입 | ①/④ | 전역 CSS 변수(`globals.css`)와 Tailwind `brand`/`accent` 토큰은 존재. **2026-07-06 Codex 디자인 토큰/컴포넌트 규칙 문서화**: [`ornscore-design-tokens-component-rules-2026-07-06.md`](./ornscore-design-tokens-component-rules-2026-07-06.md)에서 현재 색·타입·카드·컨트롤·배지 규칙과 추출 후보를 고정. 단, 브랜드 리뉴얼/팔레트 전면 교체는 여전히 ④ [제품] 결정 |
...
- **다음 "한 단계 큰 방향" 베팅 숏리스트(폴리시 웨이브 이후)** → `docs/ornscore-next-product-bets-2026-07-03.md` (Task 178). 위 §B ④·§C ⑤ 항목을 사용자 가치×차별성÷오너 게이트로 랭크(관심 그룹/메모/CSV·데이터 설명가능성·공시 파이프라인·커버리지·알림 라이브·TWA 래퍼) + 각 베팅의 "지금 가능한 ③ 첫 로컬 작업" 명시. 근시일 ③ 큐(A절)를 대체하지 않는 참고 레이어. 문서 전용·소스 무변경.
