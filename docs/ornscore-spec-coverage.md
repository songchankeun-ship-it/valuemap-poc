...
| K. 무료/유료 경계 재설계(§36~38) | ①/④ | `pricing/page.tsx` Free/Pro 경계·전환 문구·`limits.ts` 단일 출처(#36). **Premium 3티어·세부 가격은 제품 결정(④)** |
...
| O. QA 체크리스트(§47~53) | ② | #38 데스크톱/390px 시각 QA 스윕에서 12경로 점검(코드 0수정·치명 오류 0). **실 브라우저 모바일 게이트는 운영자(⑤)** |
...
| 5.4 점수 급변 알림 | ①/④ | `conditionAlerts.ts`·`cron/evaluate-alerts`·`settings/notifications`. **Task 45**: 설정 페이지에 점수 급변 알림을 미리보기 종류로 노출 + 실데이터(전일 대비 변화/없으면 형식 예시) 예시 카드(`AlertExampleCards`). **실제 발송 라이브·임계 튜닝·전일대비 급변 자동 산출은 운영/데이터 결정(④)** |
...
| 7. 알림 시스템(이메일/웹/텔레그램) | ③/④ | 이메일 cron(`notify`·`daily-insight`) 골격. **Task 45 (UX MVP 완료)**: `settings/notifications` 재구성 — 9종 알림 종류 카탈로그(`alertCatalog.ts`/`AlertTypeCatalog`, 라이브 2종+미리보기 7종), 미리보기 토글 로컬 저장(`alertPrefs.ts`, 무발송·의도적 비연결), 채널 현황 정직 표시(`NotificationChannels`, 이메일=사용 중/웹·텔레그램·카카오·앱푸시=준비 중), 실데이터 예시 카드, 비로그인도 개념 열람 + 로그인 CTA, `/watchlist` 연결 CTA. **신규 종류 실발송·웹/텔레그램/카카오 채널·종류별 Supabase 스키마는 후속(④)** |
| 8. 개인화 대시보드 | ④ [제품] | `watchlist`·`history`·`recentViews` 위젯 일부. 통합 개인화 대시보드는 제품 설계 필요 |
| 8.2 관심 종목 그룹/메모/CSV | ③/④ | `watchlist.ts` 추가/삭제·점수 델타 존재. 그룹·메모·CSV는 신규 기능 |
...
| 10. 종목 커버리지 확대(138→500→전체) | ④ [개발] | 현재 138종목. 단계적 확장은 데이터 품질·파이프라인 |
| 11. 유료화 구조(Free/Pro/Premium) | ①/④ | Free/Pro 2티어(#36). **Premium·가격 확정은 제품 결정(④)** |
| 13. 법적 리스크 관리·고지 | ②/⑤ | 고지 문구 반영(#33·#37). 결제 약관 [법무](⑤) |
| 14. 관리자 기능(대시보드·사용자·콘텐츠 관리) | ④ [개발] | 미구현. 결제 사용자·알림 발송률·콘텐츠 관리 신규 시스템 |
| 15. 기술 고도화(파이프라인·알림 인프라·결제·권한) | ④ [개발/사업] | 결제 연동·구독 권한·영수증 등 미구현 |
| 16~19. 상용화 로드맵·MVP·요금제 | ④ [제품] | 로드맵 자체가 제품/사업 결정 |
...
| 12. 상세 필터 패널(접힘·예상 결과 수) | ①/③ | 접힘 상세 필터 존재. **변경 시 "예상 결과 수" 실시간 표시는 부분 → ③(소)** |
| 13. 결과 카드 강점/주의 라벨 | ① | `deriveSignals`·결과 카드(#36) |
| 14. 결과 없음 상태(조건 완화 제안) | ① | `StocksExplorer.tsx` 빈 상태·완화 안내(#36) |
...
14. **Premium 3티어·결제/구독·권한(설계서 1 §K / 2 §11·§15)** — 가격 확정·결제 연동·권한 관리. 제품+사업 결정.
