...
| B §7 [P1-1/P1-2] Free/Pro/Premium 경계·무료 알림 제한 | ④ [제품] | Task 46 정보구조 완료(`pricing.ts` 3티어·미확정 가격). **알림을 Pro 핵심 가치로 이동·무료 알림 제한은 가격/권한 확정과 함께 제품 결정(④)** — Task 60 범위 밖 |
| B §8 종목 탐색 정보 밀도(첫 화면 검색·프리셋 강조) | ①/③ | 검색 우선·질문형 프리셋·접힘 상세 필터(#36) + 필터 텍스트 중복 제거(#60). 모바일 바텀시트 풀세트는 설계서 7 ③ |
| B §9 [P1-3] 종목 상세 초보자 카드 내부 CTA | ③ | STEP 카드 자체가 앵커 링크(`#basis` 등)로 다음 행동 연결(#60). 별도 버튼형 CTA 블록은 P1 후속 |
| B §10 [P2-1] 관리자 데이터 상태판 | ④ [개발] | 미구현(§N·§14와 동일). `/status` selfCheck 근접 |
| C §11~13 문구 조정(급등/고점추격/시장관심) | ③ | 프리셋·지표 가이드 자극적 표현 중립화 — P1/문구 후속(Task 60 P0 범위 밖) |
| D §14~15 공시 200건/분석 대상 필터 | ④/③ | 공시 전체 기간 수집(④)·홈 분석 대상만 필터(③) 후속 |
| E §16~17 백테스트 고지 중복·KPI 균형 | ①/③ | 수익/위험 균형 캡션 존재(#27). 고지 문장 중복 정리는 ③ |
