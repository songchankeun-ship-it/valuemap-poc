...
### 2026-07-28 비시장 API 추가 검토

`src/app/api/beta-feedback/route.ts`를 검토된 API 기준선에 추가했다. 이 경로는 사용자가
직접 작성한 베타 의견을 기존 `data_reports` 운영 큐에 저장할 뿐, 시장·제공자 데이터를
반환·변환·다운로드·확대·재배포하지 않는다. 신규 공개 데이터 API, 유료화, 대량 다운로드,
재배포 확대, 권리 미확인 소스 필드 추가 금지 정책은 그대로 유지한다.
