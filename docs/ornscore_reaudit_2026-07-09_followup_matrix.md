...
## Completed In Contact Cleanup Slice (Task 130)

- Public-facing contact address consolidated to the domain email `contact@ornscore.com` and the owner's personal Gmail (`songchankeun@gmail.com`) removed from all public surfaces: `/about`, `/terms`, `/privacy` (rights exercise + responsible-person contact), the data-issue report mailto source (`src/lib/dataStatus.ts` `reportEmail`, shared by `/status`·`/about`·footer), and the app/mobile submission docs (`docs/app-store-submission-pack.md`, `docs/ornscore-mobile-listing-prep-pack.md`).
- Personal name removed from privacy/legal contact copy: `/privacy` §8 책임자 now reads "개인정보 보호 책임자: 오른스코어 운영팀" and `/about` 운영 line now reads "운영: 오른스코어 운영팀" — neutral organization wording; users still reach the responsible operator via `contact@ornscore.com`.
- Notification sender `noreply@ornscore.com` (cron notify/evaluate-alerts) left unchanged — it is an outgoing-only sender, not a user contact/reply address. No new aliases invented (no support@/privacy@); only `contact@ornscore.com` used.
- Not changed (intentionally archival/internal): `CLAUDE.md` owner-profile line and prior task-history notes in `PROGRESS.md`/`docs/AI_HANDOFF.md` that record the old Gmail were once kept — these are internal handoff records, not public-facing, so they are left as history superseded by this slice.
