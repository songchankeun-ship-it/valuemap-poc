...
> 지금 무엇이 끝났고 / 무엇이 운영자만 할 수 있고 / 무엇이 아직 자동화 불가인지 **한 문서**로 보려면
> `docs/app-readiness-audit-2026-07-11.md`(현재 준비도 감사)를 먼저 본다. 이 체크리스트는 코드 마감 상태·실행 명령·배포 전 금지에 집중한다.
> 2026-07-11 기준 `npm run app:check`는 **통과**(FAIL 0), 남은 것은 외부 게이트 1개(`assetlinks.json` 실 서명 지문 대기)뿐이다.
