...
| # | 원문 항목(인용) | 카테고리(§4) | Severity(§5) | 이미 추적/완료?(spec-coverage 대조) | 운영자 전용? | 매핑 파일/문서 | 검증 게이트(§6) | task 프롬프트(§5) | 완료 기준 |
|---|---|---|---|---|---|---|---|---|---|
| 1 | | | | | | | | | |
| 2 | | | | | | | | | |
| 3 | | | | | | | | | |
...
> 새 두 칸의 목적: **이미 추적/완료?** = 방출 전에 `docs/ornscore-spec-coverage.md`(및 Task 48/최근 QA)와 대조해 중복 task를 막는 dedup 칸. **운영자 전용?** = 코드로 못 닫는 항목(§2 가드 탈락·C8·P1 운영자 게이트)을 §8로 보내는 라우팅 플래그. (드라이런 예시: `docs/ornscore-expert-feedback-dry-run-2026-07-03.md`)

### 7-A. 방출 규칙 재확인

- 각 행은 §4 가드 통과 → §5 severity → §6 게이트가 **모두** 채워져야 방출한다.
- **방출 전 dedup(중복 task 방지):** 각 행은 방출 전에 `docs/ornscore-spec-coverage.md`(및 Task 48/최근 QA)와 대조해 "이미 추적/완료?" 칸을 채운다. **이미 ① 완료**거나 백로그로 추적 중인 항목은 신규 task로 재기재하지 말고 그 칸에 근거(작업 번호·spec-coverage 절)만 남기고 드롭한다.
- **운영자 전용 라우팅:** "운영자 전용?"이 예인 행(§2 가드 탈락·C8·P1의 운영자 육안/실기기 게이트)은 코드 task로 방출하지 않고 §8 운영자 버킷으로만 보낸다.
